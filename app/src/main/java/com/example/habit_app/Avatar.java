package com.example.habit_app;

import android.content.Intent;
import android.content.SharedPreferences;
import android.os.Bundle;
import android.util.Log;
import android.view.View;
import android.widget.EditText;
import android.widget.ImageView;
import android.widget.Toast;

import androidx.appcompat.app.AppCompatActivity;

public class Avatar extends AppCompatActivity {

    private ImageView avatar1, avatar2, avatar3, avatar4;
    private EditText nicknameInput;
    private int selectedAvatarId = -1;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.choose_avatar);

        avatar1 = findViewById(R.id.avatar1);
        avatar2 = findViewById(R.id.avatar2);
        avatar3 = findViewById(R.id.avatar3);
        avatar4 = findViewById(R.id.avatar4);
        nicknameInput = findViewById(R.id.nicknameInput);

        avatar1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selectAvatar(R.drawable.avatar1);
            }
        });

        avatar2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selectAvatar(R.drawable.avatar2);
            }
        });

        avatar3.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selectAvatar(R.drawable.avatar3);
            }
        });

        avatar4.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selectAvatar(R.drawable.avatar4);
            }
        });

        findViewById(R.id.confirmButton).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                confirmSelection();
            }
        });
    }

    private void selectAvatar(int avatarId) {
        selectedAvatarId = avatarId;

        // Reset border of all avatars to indicate selection
        avatar1.setBackgroundResource(0);
        avatar2.setBackgroundResource(0);
        avatar3.setBackgroundResource(0);
        avatar4.setBackgroundResource(0);

        // Highlight the selected avatar
        if (avatarId == R.drawable.avatar1) {
            avatar1.setBackgroundResource(R.drawable.avatar_border);
        } else if (avatarId == R.drawable.avatar2) {
            avatar2.setBackgroundResource(R.drawable.avatar_border);
        } else if (avatarId == R.drawable.avatar3) {
            avatar3.setBackgroundResource(R.drawable.avatar_border);
        } else if (avatarId == R.drawable.avatar4) {
            avatar4.setBackgroundResource(R.drawable.avatar_border);
        }
    }

    private void confirmSelection() {
        String nickname = nicknameInput.getText().toString();
        // Remove newline characters
        nickname = nickname.replace("\n", "").replace("\r", "");

        if (selectedAvatarId == -1 || nickname.isEmpty()) {
            Toast.makeText(this, "Please select an avatar and enter a nickname", Toast.LENGTH_SHORT).show();
            return;
        }

        // Save selected avatar and nickname in SharedPreferences
        SharedPreferences preferences = getSharedPreferences("UserPrefs", MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        editor.putInt("AVATAR_ID", selectedAvatarId);
        editor.putString("NICKNAME", nickname);
        editor.apply();


        // Redirect to Habits page
        Intent intent = new Intent(Avatar.this, Habits.class);
        intent.putExtra("NICKNAME", nickname);
        intent.putExtra("AVATAR_ID", selectedAvatarId);
        startActivity(intent);
        finish();
    }
}
